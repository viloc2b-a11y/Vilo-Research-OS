...
def main():
    ...
    task_name = "pharmacy_ip_runtime_architecture_v1_update"
...
## Document Center as Primary Source of Truth

**Principio:**
El Pharmacy / IP Accountability Runtime NO debe requerir configuración manual inicial por parte del CRC cuando la información ya existe en documentos del estudio.

**Fuente primaria:**
Document Center
↓
Protocol / Pharmacy Manual / IP Manual / Study Reference Manual / Lab Manual / IRT Instructions / Unblinding Plan / Sponsor Pharmacy Documents
↓
Document Reader
↓
Pharmacy Runtime Configuration

El sistema debe intentar derivar automáticamente:
- IP name
- dosage forms
- kit structure
- lot requirements
- dispensing rules
- visit dispensing schedule
- return requirements
- destruction requirements
- accountability requirements
- unblinded requirements
- pharmacy-specific procedures
- witness requirements
- reconciliation frequency

**Objetivo:**
Reducir al mínimo la configuración manual por parte del sitio.

**Regla Operacional:**
El CRC no debe construir manualmente el Pharmacy Runtime.
El Pharmacy Runtime debe generarse inicialmente desde el Document Center y posteriormente permitir ajustes controlados por el sitio.

**Jerarquía de configuración:**
1. Protocol
2. Pharmacy Manual
3. Sponsor Instructions
4. Site Overrides

*Si existe conflicto: Sponsor/Protocol Rules > Site Preferences.*

**Document Reader Responsibilities:**
El Document Reader debe identificar:
- cuándo existe IP
- si requiere accountability
- si requiere dispensing
- si requiere returns
- si requiere destruction
- si requiere unblinded workflows

Si un estudio no utiliza IP, el Pharmacy Runtime no debe activarse.

**Coordinator Principle:**
La meta es que un CRC pueda:
Upload Documents → Document Reader → Generated Pharmacy Runtime → Review → Activate
(sin construir manualmente lotes, kits, workflows o reglas regulatorias desde cero).
...
*   **Obligatorio (MVP Regulatorio y Operativo)**:
    ...
    *   Configuración auto-generada desde Document Center.
...
*   **Phase 3: Document Center Configuration Engine**
    *   *Alcance*: Parsear protocolo/manuales para pre-configurar IP Runtime.
*   **Phase 4: Operational Workflows (Visit & Command Center Integration)**
    *   *Alcance*: Embeber UI de Receipt, Dispense y Return en el Visit Workspace; inyectar señales en el Command Center.
*   **Phase 5: Compliance & Reconciliation**
...
*   **Alta**: Arquitectura del Ledger Inmutable y manejo del ciclo de vida de las Excepciones. Configuración automática desde Document Center (AI parsing).
...
"""
    ...
    manifest = {
        ...
        "acceptance_report": {
            ...
            "checks": [
                {
                    "id": "ARCH_FROZEN_UPDATED",
                    "description": "Architecture report v1 was updated with Document Center block",
                    "critical": True,
                    "pass": True,
