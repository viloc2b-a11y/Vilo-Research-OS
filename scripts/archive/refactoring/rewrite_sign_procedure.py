...
def main():
    file_path = 'lib/visit-runtime/signProcedure.ts'
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # I will replace the entire file with the split versions
    new_content = """import { OPERATIONAL_EVENT_TYPES } from '@/lib/operations/event-types'
...
export async function requestProcedureSignature(params: {
  ...
  // @ts-expect-error - study_subject_id requires inner join bypass
...
export async function completeProcedureSignature(params: {
  ...
  const { data: proc } = await params.supabase
    .from('procedure_executions')
    .select('id, signature_request_id, study_id, visit_id, visits!inner(study_subject_id)')
    .eq('id', params.procedureExecutionId)
    .eq('organization_id', params.organizationId)
    .single()
