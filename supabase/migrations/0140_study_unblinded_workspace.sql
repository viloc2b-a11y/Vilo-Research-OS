...
CREATE TABLE public.study_ip_accountability (
...
CREATE INDEX idx_study_ip_accountability_org ON public.study_ip_accountability(organization_id);
CREATE INDEX idx_study_ip_accountability_study ON public.study_ip_accountability(study_id);

-- 2. IP Dispensing / Preparation Log
CREATE TABLE public.study_ip_dispensing (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL REFERENCES public.organizations(id) ON DELETE CASCADE,
  study_id uuid NOT NULL REFERENCES public.studies(id) ON DELETE CASCADE,
...
CREATE INDEX idx_study_ip_dispensing_org ON public.study_ip_dispensing(organization_id);
CREATE INDEX idx_study_ip_dispensing_study ON public.study_ip_dispensing(study_id);
CREATE INDEX idx_study_ip_dispensing_subject ON public.study_ip_dispensing(subject_id);
