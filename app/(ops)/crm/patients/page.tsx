        ...
        <section className="space-y-4">
          ...
          {detail ? (
            <div className="space-y-4 rounded-md border border-slate-200 bg-white p-4">
              ...
              <form action={linkLeadToSubjectAction} className="rounded-md border border-teal-200 bg-teal-50 p-3 space-y-3">
                <input type="hidden" name="organizationId" value={organizationId} />
                <input type="hidden" name="patientLeadId" value={detail.lead?.id ?? ''} />
                <div>
                  <h3 className="text-sm font-semibold text-slate-900">Link to subject</h3>
                  {detail.lead?.linkedSubjectId ? (
                    <p className="mt-1 text-xs text-teal-700">
                      Currently linked: <span className="font-mono">{detail.lead.linkedSubjectId}</span>
                    </p>
                  ) : (
                    <p className="mt-1 text-xs text-slate-500">
                      Link this lead to a study subject to close the recruitment loop.
                    </p>
                  )}
                </div>
                <label className="space-y-1 text-xs font-medium text-slate-600">
                  ...
                  <input
                    name="studySubjectId"
                    className="h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm font-mono"
                    placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
                    required
                  />
                </label>
                <button type="submit" className="rounded-md bg-teal-700 px-3 py-2 text-sm font-semibold text-white hover:bg-teal-800">
                  {detail.lead?.linkedSubjectId ? 'Update subject link' : 'Link to subject'}
                </button>
              </form>
            </div>
          ) : (
