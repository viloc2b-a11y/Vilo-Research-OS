...
import { PartnerListTable } from '@/components/recruitment-partners/PartnerListTable'
...
export default async function PartnerListPage() {
  ...
  if (!organizationId) {
    return (
      <div className="space-y-3 p-6">
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Partner Management</h1>
  ...
  const partners = await loadPartnerList(supabase, organizationId)

  return (
    <div className="space-y-6 p-6">
      <header className="max-w-4xl">
        <p className="text-xs font-semibold uppercase tracking-wide text-teal-700">Patient Acquisition</p>
        <h1 className="mt-1 text-2xl font-semibold tracking-tight text-slate-900">Partner Management</h1>
        <p className="mt-2 text-sm text-slate-600">
          Track and manage recruitment partners that drive campaign lead generation.
        </p>
      </header>

      <PartnerListTable partners={partners} canManage={canManage} />
    </div>
  )
}
