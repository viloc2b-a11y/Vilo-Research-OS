...
import { OperationalSignaturesClient } from '@/components/operational-signatures/operational-signatures-client'

type OperationalSignaturesPageProps = {
...
async function OperationalSignaturesContent({
  initialStudyId,
}: {
  ...
  if (!organizationId) {
    return (
      <div className="space-y-4 p-6">
        <h1 className="text-2xl font-semibold tracking-tight">Operational eSignature</h1>
  ...
  const canAccess =
    canAccessSubjectVisitWorkspace(memberships, organizationId)
    || canMutateOrganizationData(memberships, organizationId)
  if (!canAccess) {
    return (
      <div className="space-y-4 p-6">
        <h1 className="text-2xl font-semibold tracking-tight">Operational eSignature</h1>
...
export default async function OperationalSignaturesPage({
  searchParams,
}: OperationalSignaturesPageProps) {
  const params = await searchParams
  const initialStudyId = params.study_id?.trim() || null

  return (
    <Suspense fallback={<LoadingFallback />}>
      <OperationalSignaturesContent initialStudyId={initialStudyId} />
    </Suspense>
  )
}
