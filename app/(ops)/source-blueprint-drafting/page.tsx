...
import { DraftSuggestionQueue } from '@/components/source-blueprint-drafting/draft-suggestion-queue'

type SourceBlueprintDraftingPageProps = {
...
async function SourceBlueprintDraftingContent({
  initialStudyId,
}: {
  ...
  if (!organizationId) {
    return (
      <div className="space-y-4 p-6">
        <h1 className="text-2xl font-semibold tracking-tight">Source Blueprint Drafting</h1>
...
export default async function SourceBlueprintDraftingPage({
  searchParams,
}: SourceBlueprintDraftingPageProps) {
  const params = await searchParams
  const initialStudyId = params.study_id?.trim() || null

  return (
    <Suspense fallback={<LoadingFallback />}>
      <SourceBlueprintDraftingContent initialStudyId={initialStudyId} />
    </Suspense>
  )
}
