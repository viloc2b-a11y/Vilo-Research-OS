...
export async function GET(request: Request, context: RouteContext) {
  ...
  const parsed = parseFindingsListQuery(id, url.searchParams)
  if (!parsed.ok) {
    return jsonEnvelope(
  ...
  try {
    const envelope = await callSourceRpc(ctx.supabase, 'list_source_response_set_findings', {
      p_organization_id: query.organization_id,
      p_source_response_set_id: query.source_response_set_id,
      p_active_only: query.active_only,
    ...
    return jsonEnvelope(
      fromRpcThrown(err, { requestId: ctx.requestId, rpc: 'list_source_response_set_findings' }),
    )
  }
}
