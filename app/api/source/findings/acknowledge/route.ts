...
export async function POST(request: Request) {
  ...
  const parsed = parseFindingActionBody(raw)
  if (!parsed.ok) {
    return jsonEnvelope(
  ...
  try {
    const envelope = await callSourceRpc(ctx.supabase, 'acknowledge_source_validation_finding', {
    ...
    return jsonEnvelope(
      fromRpcThrown(err, { requestId: ctx.requestId, rpc: 'acknowledge_source_validation_finding' }),
    )
  }
}
