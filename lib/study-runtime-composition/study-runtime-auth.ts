...
export type StudyRuntimeAuthContext =
  | {
      ok: true
      userId: string
...
export async function authorizeStudyRuntimeRead(
  organizationId: string,
): Promise<StudyRuntimeAuthContext> {
...
export async function authorizeStudyRuntimeWrite(
  organizationId: string,
): Promise<StudyRuntimeAuthContext> {
  ...
  if (!canManageSourceBuilder(auth.memberships, organizationId)) {
    return {
      ok: false,
      status: 403,
      message: 'Your role does not allow study runtime composition changes.',
    }
  }
