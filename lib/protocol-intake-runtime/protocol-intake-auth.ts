...
export type ProtocolIntakeAuthContext =
  | {
      ok: true
      userId: string
...
export async function authorizeProtocolIntakeRead(
  organizationId: string,
): Promise<ProtocolIntakeAuthContext> {
...
export async function authorizeProtocolIntakeWrite(
  organizationId: string,
): Promise<ProtocolIntakeAuthContext> {
  ...
  if (!canManageSourceBuilder(auth.memberships, organizationId)) {
    return {
      ok: false,
      status: 403,
      message: 'Your role does not allow protocol intake runtime management.',
    }
  }
