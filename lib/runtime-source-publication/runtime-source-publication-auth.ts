...
export type RuntimeSourcePublicationAuthContext =
  | { ok: true; userId: string; memberships: OrganizationMembership[] }
  | { ok: false; status: number; message: string }

export async function authorizeRuntimeSourcePublicationRead(
  organizationId: string,
): Promise<RuntimeSourcePublicationAuthContext> {
...
export async function authorizeRuntimeSourcePublicationWrite(
  organizationId: string,
): Promise<RuntimeSourcePublicationAuthContext> {
  ...
  if (!canManageSourceBuilder(auth.memberships, organizationId)) {
    return {
      ok: false,
      status: 403,
      message: 'Your role does not allow runtime source publication.',
    }
  }
