...
export type ProtocolRuntimeGenerationAuthContext =
  | { ok: true; userId: string; memberships: OrganizationMembership[] }
  | { ok: false; status: number; message: string }

export async function authorizeProtocolRuntimeGenerationRead(
  organizationId: string,
): Promise<ProtocolRuntimeGenerationAuthContext> {
...
export async function authorizeProtocolRuntimeGenerationWrite(
  organizationId: string,
): Promise<ProtocolRuntimeGenerationAuthContext> {
  ...
  if (!canManageSourceBuilder(auth.memberships, organizationId)) {
    return {
      ok: false,
      status: 403,
      message: 'Your role does not allow protocol runtime generation.',
    }
  }
