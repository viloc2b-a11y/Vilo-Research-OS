...
export async function generatePrintableSourcePacket(supabase: SupabaseClient, runId: string) {
  try {
    ...
    // 3. Resolve evidence
    const evidence = await EvidenceResolver.resolveForPrintableSourcePacket(supabase, adaptedRun)

    // 4. Render React to HTML
    const htmlContent = renderToStaticMarkup(
      PrintableSourcePacketTemplate({
        evidence,
        runId: run.id,
    ...
    const fileName = `VILO_Source_V${evidence.visitInfo.visitName.replace(/[^a-zA-Z0-9]/g, '')}_${evidence.subjectIdentifier}.pdf`
